...
private let recordingLog = OSLog(subsystem: "app.jottr", category: "Recording")
...
final class AppState: ObservableObject, @unchecked Sendable {
    ...
    private let customPromptL1StorageKey = "customPrompt_L1"
    private let customPromptL2StorageKey = "customPrompt_L2"
    private let customPromptL3StorageKey = "customPrompt_L3"
    private let customPromptL4StorageKey = "customPrompt_L4"
    ...
    @Published var customPromptL1: String {
        didSet { UserDefaults.standard.set(customPromptL1, forKey: customPromptL1StorageKey) }
    }

    @Published var customPromptL2: String {
        didSet { UserDefaults.standard.set(customPromptL2, forKey: customPromptL2StorageKey) }
    }

    @Published var customPromptL3: String {
        didSet { UserDefaults.standard.set(customPromptL3, forKey: customPromptL3StorageKey) }
    }

    @Published var customPromptL4: String {
        didSet { UserDefaults.standard.set(customPromptL4, forKey: customPromptL4StorageKey) }
    }

    func customPrompt(for level: IntensityLevel) -> String {
        switch level {
        case .l1: return customPromptL1
        case .l2: return customPromptL2
        case .l3: return customPromptL3
        case .l4: return customPromptL4
        }
    }

    func setCustomPrompt(_ prompt: String, for level: IntensityLevel) {
        switch level {
        case .l1: customPromptL1 = prompt
        case .l2: customPromptL2 = prompt
        case .l3: customPromptL3 = prompt
        case .l4: customPromptL4 = prompt
        }
    }
    ...
    @Published var keepMicIndicatorAlive: Bool {
        didSet { UserDefaults.standard.set(keepMicIndicatorAlive, forKey: "keepMicIndicatorAlive") }
    }
    ...
    init() {
        ...
        let customPromptL1 = UserDefaults.standard.string(forKey: customPromptL1StorageKey) ?? ""
        let customPromptL2 = UserDefaults.standard.string(forKey: customPromptL2StorageKey) ?? ""
        let customPromptL3 = UserDefaults.standard.string(forKey: customPromptL3StorageKey) ?? ""
        let customPromptL4 = UserDefaults.standard.string(forKey: customPromptL4StorageKey) ?? ""
        ...
        let rawLevel = UserDefaults.standard.string(forKey: "intensityLevel") ?? "L2"
        let intensityLevel = IntensityLevel(rawValue: rawLevel) ?? .l2
        ...
        self.customPromptL1 = customPromptL1
        self.customPromptL2 = customPromptL2
        self.customPromptL3 = customPromptL3
        self.customPromptL4 = customPromptL4
        ...
        if let stored = UserDefaults.standard.object(forKey: "keepMicIndicatorAlive") as? Bool {
            self.keepMicIndicatorAlive = stored
        } else {
            self.keepMicIndicatorAlive = true
        }
        self.snippetCount = snippetStore.load().count
    ...
    func addSnippet(trigger: String, expansion: String) {
        ...
        do {
            ...
            snippetCount = snippetStore.load().count
        } catch {
    ...
    func updateSnippet(id: UUID, trigger: String, expansion: String) {
        do {
            try snippetStore.update(id: id, trigger: trigger, expansion: expansion)
            snippetCount = snippetStore.load().count
        } catch {
    ...
    func startAccessibilityPolling() {
        ...
        // Silent check only — never call SCShareableContent here as it triggers the system prompt
        ...
        accessibilityTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) { [weak self] _ in
            DispatchQueue.main.async {
                ...
                if trusted && !self.hasAccessibility {
                    // Permission was just granted — restart monitors so global event
                    // handlers (hotkey, intensity shortcuts) register successfully.
                    self.intensityShortcutManager.start()
                    self.hotkeyManager.start(option: self.selectedHotkey)
                }
                self.hasAccessibility = trusted
                // Silent check only — SCShareableContent triggers the system dialog, never poll with it
    ...
    private func beginRecording() {
        ...
        initTimer.setEventHandler { [weak self] in
            ...
            self.overlayManager.setIntensityLevel(self.intensityLevel)
        }
        initTimer.resume()
    ...
    func showMicrophonePermissionAlert() {
        ...
        alert.informativeText = "Jottr cannot record audio without Microphone access.\n\nGo to System Settings > Privacy & Security > Microphone and enable Jottr."
    ...
    func showAccessibilityAlert() {
        ...
        alert.informativeText = "Jottr cannot type transcriptions without Accessibility access.\n\nGo to System Settings > Privacy & Security > Accessibility and enable Jottr."
        alert.alertStyle = .critical
        alert.addButton(withTitle: "Open System Settings")
        alert.addButton(withTitle: "Dismiss")
    ...
    private func stopAndTranscribe() {
        ...
        Task {
            do {
                ...
                let snippetTranscript = SnippetEngine.apply(snippets, to: rawTranscript)
                ...
                do {
                    let postProcessingResult = try await postProcessingService.postProcess(
                        ...
                        customSystemPrompt: intensityLevel.systemPrompt
                    )
                    ...
                    finalTranscript = snippetTranscript
    ...
    private func handleScreenshotCaptureIssue(_ message: String?) {
        ...
        // Screen capture is optional — never stop recording over a permission error.
        // Dictation continues without screenshot context; notify once via menu bar status.
    ...
    private func showScreenshotPermissionAlert(message: String) {
        ...
        alert.informativeText = "\(message)\n\nJottr requires Screen Recording permission to capture screenshots for context-aware transcription.\n\nGo to System Settings > Privacy & Security > Screen Recording and enable Jottr."
        alert.alertStyle = .critical
        alert.addButton(withTitle: "Open System Settings")
        alert.addButton(withTitle: "Dismiss")
