    ...
    var body: some View {
        VStack(spacing: 4) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text("Snippets (\(appState.snippetCount))")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .padding(.horizontal, 16)
                ...
                HStack(spacing: 6) {
                    ...
                    TextField("Expansion", text: $newExpansion)
                        .textFieldStyle(.roundedBorder)
                    Button("Save") {
                        let trimmedTrigger = newTrigger.trimmingCharacters(in: .whitespaces)
                        guard !trimmedTrigger.isEmpty else { return }
                        appState.addSnippet(trigger: trimmedTrigger, expansion: newExpansion)
                        newTrigger = ""
                        newExpansion = ""
                    }
            ...
            Menu("Intensity") {
                ForEach(IntensityLevel.allCases) { level in
                    Button {
                        ...
                        if appState.intensityLevel == level {
