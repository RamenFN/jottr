...
class AudioRecorder: NSObject, ObservableObject {
    ...
    private let audioFileQueue = DispatchQueue(label: "app.jottr.audiofile")
    ...
    /// Called after each recording to decide whether to keep the AVAudioEngine running.
    /// Keeping it alive preserves the orange mic indicator; tearing it down removes it.
    var shouldKeepEngineAlive: () -> Bool = { true }
    ...
    func startRecording(deviceUID: String? = nil) throws {
        ...
        if let _ = audioEngine, currentDeviceUID == deviceUID {
            ...
            // Disable voice processing to prevent macOS from switching the system
            // audio pipeline into communication mode (which muffles music playback).
    ...
    private func installTap(on inputNode: AVAudioInputNode, format: AVAudioFormat) {
        inputNode.installTap(onBus: 0, bufferSize: 4096, format: format) { [weak self] buffer, _ in
            ...
            // Always write buffered audio to disk while file is open.
            // This runs even after _recording is cleared so in-flight buffers
            // aren't dropped when stopRecording() is called.
            //
            // Use async (not sync) so the CoreAudio realtime thread never
            // blocks on file I/O. stopRecording() uses audioFileQueue.sync
            // to close the file, which drains all pending async writes first
            // — so the file is still complete before transcription starts.
            ...
            // Level meter, logging, and ready callback only while recording.
    ...
    func stopRecording() -> URL? {
        ...
        // Close the file — audioFileQueue.sync drains all pending async writes
        // before setting audioFile to nil, so no buffers are lost.
        // Engine keeps running to avoid CoreAudio graph teardown, which would
        // cause a brief audio interruption in other apps (music, video, etc.).
        // Side effect: the orange mic indicator stays on while the app is running.
        audioFileQueue.sync { audioFile = nil }

        // If user opted out of persistent mic indicator, tear down the engine
        // to dismiss the orange mic dot. The engine will be rebuilt on next startRecording().
        if !shouldKeepEngineAlive() {
            audioEngine?.inputNode.removeTap(onBus: 0)
            audioEngine?.stop()
            audioEngine = nil
