...
class AudioRecorder: NSObject, ObservableObject {
    ...
    func startRecording(deviceUID: String? = nil) throws {
        ...
        if let _ = audioEngine, currentDeviceUID == deviceUID {
            ...
            // Install tap — always writes to file while audioFile is open,
            // uses _recording only to gate level meter and ready callback.
    ...
    func stopRecording() -> URL? {
        ...
        // Stop BEFORE closing the file so any in-flight tap callbacks (which no longer
        // check _recording for writes) can flush their last buffers to disk.
        audioEngine?.stop()
        os_log(.info, log: recordingLog, "engine stopped (mic indicator off)")

        // Close file only after engine is stopped — captures all flushed audio.
