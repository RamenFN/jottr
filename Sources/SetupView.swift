...
struct SetupView: View {
    ...
    private var readingScripts: [IntensityLevel: String] {
        [
            .l1: "Um, I just wanted to say that the, uh, meeting tomorrow is at three pm and we should probably bring the, you know, the project files.",
            .l2: "So I was thinking, we should probably update the onboarding docs because new people keep asking the same questions and it's kind of slowing things down for everyone on the team.",
            .l3: "I want to write something for the team about why we changed the release process. The old way was we'd just push to main whenever, but now we have a staging step which means there's an extra review before anything goes live, and I think that's actually a good thing even though it takes more time.",
            .l4: "Okay so my CPC is 50 cents, LTV is $400, CAC is $200, so payback period is like 400 days which is way too long, and I'm wondering if I should increase budget because the unit economics look bad but volume might fix it, or if I should just focus on improving the LTV by adding a retention email sequence."
        ]
    }
    ...
    var screenRecordingStep: some View {
        VStack(spacing: 20) {
            ...
            Text("It needs this permission to see which app you're working in and any in-progress work. Nothing is stored on Jottr's servers (Jottr doesn't have servers).")
                .multilineTextAlignment(.center)
                .foregroundStyle(.secondary)
                .font(.callout)
                .fixedSize(horizontal: false, vertical: true)
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 6) {
                    ForEach(IntensityLevel.allCases, id: \.self) { level in
                        Button(action: { selectedTestLevel = level }) {
                            Text(level.rawValue)
                ...
                case .done:
                    VStack(spacing: 16) {
                        ...
                        if let error = testError {
                            ...
                            Text("Perfect — Jottr is ready to go.")
                                .font(.title2)
                                .fontWeight(.semibold)
    ...
    func fetchIfNeeded() async {
        ...
        do {
            ...
            if count > 0 {
                ...
                let stargazersURL = URL(string: "https://api.github.com/repos/RamenFN/jottr/stargazers?per_page=\(perPage)&page=\(lastPage)")!
